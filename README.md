...
$`f_\theta`$, Section 3.4 gives $`\Pi`$, and Section 3.5 the refinement.
...
count $`300 \le N \le 500`$. Each region carries sixteen image-derived
descriptors,
...
are obtained together from one pass over $`\ell`$. Non-local edges connect each region
...
would repeat identical work and add $`n`$ edge MLPs to the parameter count; (17) is
evaluated once per forward pass regardless of $`n`$.
...
```math
A_k = \frac{\mathrm{cov}_{\omega_k}\big( I_g,\, q^c \big)}
...
otherwise the more likely side is passed as a probable label, leaving GrabCut free
to move the contour inside it.
...
the min-cut of (28) is undefined, and the $`\lceil 0.1 N \rceil`$ regions with the
highest prior score on the missing side are promoted to the probable label of that
side,
...
independent per-region decisions, and taking the coverage ratio $`\rho_i`$ of (30)
rather than the thresholded label as its target lets boundary regions contribute a
graded signal. The total
objective is
...
second the pairwise contrast (10) together with the non-local neighbour search (8). Since $`300 \le N \le 500 \ll HW`$, the pixel term dominates. Testing each
...
computed once, and the head contributes $`O(D^2)`$ rather than $`O(n D^2)`$, because
the fusion (21) is by weight rather than by concatenation. At $`D = 96`$,
...
The fusion weights $`\mathbf{w}`$ of (21) are informative about depth: after training the
